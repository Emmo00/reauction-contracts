...
contract Deploy is Script {
    ...
    function run() external {
        ...
        address proxy = Upgrades.deployTransparentProxy(
            "Auction.sol",
            config.owner,
            abi.encodeCall(Auction.initialize, (config.collectible, config.usdc, config.treasury, config.owner))
        );
...
contract Upgrade is Script {
    ...
    function run() external {
        ...
        // TODO: Remove after deployment on mainnet
        ...
        Upgrades.upgradeProxy(proxy, "Auction.sol", "", opts);
